...
2. **Lecturas del servidor.** `siteCacheOptions` (`src/shared/lib/cacheTags.ts`) lee la
   ...
   **Sin precarga.** En una demo, `proxy.ts` responde 204 a la precarga de un `<Link>`
   ...
   menú. La página se pide al hacer clic (la navegación sigue siendo suave). Next solo le deja
   ver esa cabecera al proxy con `skipProxyUrlNormalize` en `next.config.ts`.
